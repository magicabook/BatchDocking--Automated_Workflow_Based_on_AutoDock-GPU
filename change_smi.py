...
def change_smi():
    ...
    for smi_txt in ligand_smi_txt_list:  # make ligand list 阅读该目录下的所有文件并存储到一个list中
        ...
        if no_work_smi != True:
            if smi_txt.endswith('.txt'):  # checking file type 检查文件后缀是否正确
                # read smi.txt
                # 开始阅读小分子集的.txt文件
                smi_path = os.path.join(ligand_smi_path, smi_txt) # 该目录下所有.txt文件的路径
                ...
                error_smi_path = os.path.join(ligand_smi_path, smi_txt)  # fund error file list # 获取错误文件的路径
